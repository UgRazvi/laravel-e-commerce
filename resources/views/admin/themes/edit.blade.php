...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-lg-8 col-md-10 col-sm-12">
            <h1 class="mb-4 text-center">Edit Theme: {{ $theme_instance->key }}</h1>

            <form action="{{ route('themes.update', $theme_instance->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group mb-4">
                    ...
                    <input type="text" name="key" id="key" class="form-control form-control-lg" value="{{ $theme_instance->key }}" required placeholder="Enter the theme key">
                </div>
                ...
                <div class="form-group mb-4">
                    ...
                    <select name="type" id="type" class="form-control form-control-lg" required>
                        <option value="color" {{ $theme_instance->type == 'color' ? 'selected' : '' }}>Color</option>
                        <option value="logo" {{ $theme_instance->type == 'logo' ? 'selected' : '' }}>Logo (Image)</option>
                        <option value="text" {{ $theme_instance->type == 'text' ? 'selected' : '' }}>Text</option>
                    </select>
                </div>
                ...
                <div class="form-group mb-4" id="value-container">
                    ...
                    <input type="text" name="value" id="value" class="form-control form-control-lg" value="{{ $theme_instance->value }}" required placeholder="Enter the value">
                </div>
                ...
                <!-- Submit Button -->
                <button type="submit" class="btn btn-primary btn-lg w-100">Update Theme</button>
            </form>
        </div>
    </div>
</div>
